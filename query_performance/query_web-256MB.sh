...
loc_result_file="loc_query_performance_web-256MB.txt"
repetitions=5
fac_dens=1

rm -f ${result_file} ${loc_result_file}
...
for i in `seq 1 ${repetitions}`; do
	for pattern_len in 4 10 20 40 100; do
		for occ in "1 1" "8 12" "75 125" "750 1250" "7500 12500"; do 
			...
			echo "${pattern_file}"
			${bin_dir}/rosa_sd2_delta --input_file=${file} --threshold=${threshold} --output_dir=${output_dir} \
							   --benchmark_loc --pattern_file=${pattern_file} --fac_dens=${fac_dens} >> ${loc_result_file}
		done
	done
done
