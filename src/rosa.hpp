...
class rosa{
	...
	private:
		...
		fringe_type				m_fringe; // type of the fringe
		size_type				m_fringe_len; // length of the fringe
		 ...
		 *        * store the depth d of the block, so that the fringe of the first
	     *          entry is shifted
	     *        * subtract and add d from the lcp values
	     *        * use a Huffman coder as io_wrapper for m_fringe	 
		 */
		class disk_block{
			...
			int_vector<8> m_fringe; // A char vector for the fringe characters.
			...
			const int_vector<8> &fringe;// const reference to the fringe characters

			disk_block():header(m_header), lcp(m_lcp), sa(m_sa), fringe(m_fringe){};
			...
			template<class tCst>
			void insert_fringe_chars(const tCst &cst, int_vector<8> &fringe, const unsigned char *text,
					                 size_type fringe_pos, size_type text_pos, size_type fringe_len){
				for (size_type i=0; i < fringe_len; ++i, ++text_pos){
					if ( text_pos < cst.csa.size() ){
						fringe[fringe_pos*fringe_len+i] = text[text_pos]; 
					}else{
						fringe[fringe_pos*fringe_len+i] = '\0';
					}
				}		
			}
			...
			void set_content(const tCst &cst, const unsigned char *text, size_type lb, size_type rb, 
					         fringe_type fringe, size_type fringe_len){
				size_type max_sa = 0;
				size_type max_lcp = 0;
...
//				std::cout << "set_context m_sa.size()=" << m_sa.size() << endl;
				...
				m_fringe.resize( (rb-lb+1)*fringe_len );
				if ( RIGHT_FRINGE == fringe and fringe_len > 0 ){
					for (size_type i=lb; i <= rb; ++i){
						size_type text_pos = t_csa[i-lb] + t_lcp[i-lb]; // for i=lb, we could also take d instead of cst.lcp[i]
						insert_fringe_chars(cst, m_fringe, text, i-lb, text_pos, fringe_len);
					}
				}
			}
			...
			size_type serialize(std::ostream &out)const{
				...
				written_bytes += m_fringe.serialize(out);
	...
	public:
		...
		const fringe_type &fringe;				//!< Type of the fringe. 
		const size_type &fringe_len;			//!< Length of the fringe.
		...
		static string get_ext_idx_filename(const char* file_name, size_type b, size_type fringe_len, 
										   fringe_type fringe, const char *output_dir=NULL){
			return get_output_dir(file_name, output_dir) + "/" + util::basename(file_name) 
				   + "." + util::to_string(b) + "."+util::to_string(fringe_len)+"."+util::to_string(fringe)+".ext_idx";
		}

		//! Get the name of the file where the external part of the index is stored.
		string get_ext_idx_filename(){
			return get_ext_idx_filename(m_file_name.c_str(), m_b, m_fringe_len, m_fringe, m_output_dir.c_str());
		}


		//! Get the name of the file where the in-memory part of the index is stored.
		static string get_int_idx_filename(const char* file_name, size_type b, size_type fringe_len, 
										   fringe_type fringe, const char *output_dir=NULL){

			return get_output_dir(file_name, output_dir) + "/" + util::basename(file_name) 
				   + "." + util::to_string(b) + "."+util::to_string(fringe_len)+"."+util::to_string(fringe)+"."+INDEX_SUF+".int_idx";
		}

		//! Get the name of the file where the in-memory part of the index is stored.
		string get_int_idx_filename(){
			return get_int_idx_filename(m_file_name.c_str(), m_b, m_fringe_len, m_fringe, m_output_dir.c_str());
		}
		 ...
		 *	\param fringe_type		Type of the fringe: Either NO_FRINGE or RIGHT_FRINGE are supported.
		 *	\param fringe_length	Length of the fringe.
		 ...
		 */
		rosa(const char *file_name=NULL, size_type b=4000, fringe_type fringe=NO_FRINGE, size_type fringe_length=0,
				bool output_tikz=false, bool delete_tmp=false, const char *tmp_file_dir="./", const char *output_dir=NULL):m_b(b)
															,m_fringe(fringe), m_fringe_len(fringe_length), m_buf_size(1024)
...
std::cout<<"id="<<util::basename(m_file_name)<<std::endl;				
			...
			size_type size_of_full_fringe = 0; // number of fringe characters we would need to
			...
			cout << "# size_of_full_fringe_for_all_blocks = " << size_of_full_fringe << endl; 
#endif			
			vector<block_node> v_block;
			get_block_info(fwd_cst, fwd_bf, b,red_blocks, red_blocks_delta0, 
		            	   singleton_blocks, elements_in_irred_blocks, size_of_full_fringe, v_block);
			...
			cout << "# size_of_full_fringe_for_red_blocks = " << size_of_full_fringe << endl; 
...
ext_idx_out.rdbuf()->pubsetbuf(output_buf, output_buf_size);				
					...
					char *text = NULL;
					if ( m_fringe_len > 0 ){ // we need the text, if we would like to add fringe characters
					...
					for (size_t fwd_id=0; fwd_id < external_block.size(); ++fwd_id){
						if ( external_block[fwd_id].size() > 0 ){
							...
							db.set_content( fwd_cst, (const unsigned char*)text, fwd_bf_select(fwd_id+1), fwd_bf_select(fwd_id+2)-1, m_fringe, m_fringe_len );
...
/*						
std::cout<<"pattern="<<pattern<<" matched so far="<<d<<std::endl;
std::cout<<"bwd_id="<<bwd_id<<std::endl;
*/
						size_type block_addr = m_pointer[bwd_id];
/*						
std::cout<<"block_addr="<<block_addr<<std::endl;
std::cout<<"m_X="<<m_X<<" m_count_queries="<<m_count_queries<<std::endl;
if ( !m_ext_idx ){
	std::cout<<"ERROR: the stream for the external index is not opened" <<std::endl;
}
*/
						if ( m_fringe_len == 0 ){
							return search_block(pattern+d, m-d, d, rb+1-lb, bwd_id, block_addr);
						}else{
							return search_block_right_fringe(pattern+d, m-d, d, rb+1-lb, bwd_id, block_addr);
						}
					}
				}
			}
...
std::cout<<"c="<<c<<" m_C1["<<*cp<<"]="<<m_C1[*cp]<<std::endl;	
		...
		//! Search for a pattern with the help of right-justified fringe characters
		/*!
		 * \sa search_block
		 * \par Time complexity
		 *		\f$ \Order{min(b, m)} \f$ disk accesses in the worst case
		 */
		size_type search_block_right_fringe(const unsigned char *pattern, size_type m, size_type depth, size_type size,
				               size_type bwd_id, size_type block_addr)const{
			disk_block db;
#ifdef DEBUG_DISK_ACCESS
			m_ext_idx.seekg(0, std::ios::end);
			std::streampos end_pos = m_ext_idx.tellg(); // get the end position
			m_ext_idx.seekg(0, std::ios::beg);
			std::streampos start_pos = m_ext_idx.tellg(); // get the end position
			cout << "end addr = " << end_pos-start_pos << endl;
#endif			
			seekg(m_ext_idx, block_addr);
#ifdef DEBUG_DISK_ACCESS
			if ( m_count_queries == m_X ){
				cout<<"seeked to block addr "<<block_addr<<endl;
				cout<<"size = "<<size<<endl;
				cout << "bwd_id = " << bwd_id << " in [0," << m_k << "]" << endl;
			}
#endif			
			db.load(m_ext_idx);
#ifdef DEBUG_DISK_ACCESS
			if ( m_count_queries == m_X ){
				cout<<"disk block loaded"<<block_addr<<endl;
			}
#endif		
			size_type delta_x = 0;
			size_type delta_d = 0;
...
#ifdef OUTPUT_STATS
			if ( m_count_queries == m_X ){
				cout<<"match pattern "<<pattern<<" m="<<m<<" depth="<<depth<<" size="<<size<<endl;
			}
#endif			
			// do a linear scan and use the fringe to search for the pattern
			size_type lb = delta_x, end = delta_x+size-1; // [lb..end]
			size_type matched = depth+delta_d;
#ifdef OUTPUT_STATS
			if ( m_count_queries == m_X ){
...
//			for ( ;lb <= end and m > 0; ++lb){
			while ( m > 0 and lb <= end /*and matched <= db.lcp[lb]*/ ) {
#ifdef OUTPUT_STATS
if ( m_count_queries == m_X ){				
	m_text.seekg(db.sa[lb], std::ios::beg );
	char *buf = new char[db.lcp[lb]+m_fringe_len+1];
	size_type Xlen = db.lcp[lb]+m_fringe_len;
	if ( Xlen >  m_n-db.sa[lb]){
		Xlen = m_n-db.sa[lb];
	}
	m_text.read(buf, Xlen);
	buf[Xlen]='\0';
	cout << "text["<<lb<<"..]="<<buf<< endl;
	delete buf;
}
#endif				
//				cout << "lb="<<lb<<" matched="<<matched<<" dp.lcp[lb]+m_fringe_len="<<db.lcp[lb]+m_fringe_len << endl;
				if ( db.lcp[lb]+m_fringe_len <= matched ){ // 
#ifdef OUTPUT_STATS
	if ( m_count_queries == m_X ){				
		cout << "(1) db.lcp["<<lb<<"]+m_fringe="<<db.lcp[lb]<<"+"<<m_fringe_len<<" <= "<< matched << endl;
	}
#endif					
					size_type more_matched = 0;
					match_pattern(pattern, m, db.sa[lb] + matched, more_matched);
#ifdef OUTPUT_STATS
	if ( m_count_queries == m_X ){				
		cout << "more_matched=" << more_matched << endl;
	}
#endif					
					pattern += more_matched;
					matched += more_matched;
					m -= more_matched;
//cout<<"...m="<<m<<endl;					
//				cout << "lb="<<lb<<" matched="<<matched<<" dp.lcp[lb]+m_fringe_len="<<db.lcp[lb]+m_fringe_len << endl;
				}else{ // db.lcp[lb]+m_fringe_len > matched
					if ( matched < db.lcp[lb] ){ // gap in the matching => we have to go to the text
#ifdef OUTPUT_STATS
	if ( m_count_queries == m_X ){
		cout << "(2) db.lcp["<<lb<<"]+m_fringe_len="<<db.lcp[lb]<<"+"<<m_fringe_len << " > " <<matched << endl;
		cout << " and " << matched << " < "<<db.lcp[lb] << endl;
	}				
#endif						
						size_type more_matched = 0;
						match_pattern(pattern, m, db.sa[lb] + matched, more_matched);
	...
	if ( m_count_queries == m_X ){				
		cout << "more_matched=" << more_matched << endl;
						...
						matched += more_matched;
						m -= more_matched;	
					}else{ // no gap, we first check the fringe
//						matched < db.lcp[lb] + m_fringe_len
//                      matched >= db.lcp[lb]						
#ifdef OUTPUT_STATS
	if ( m_count_queries == m_X ){
		cout << "(3) db.lcp["<<lb<<"]+m_fringe_len="<<db.lcp[lb]<<"+"<<m_fringe_len << " > " <<matched << endl;
		cout << " and " << matched << " >= "<<db.lcp[lb] << endl;
	}	
#endif						
						size_type fp = matched - db.lcp[lb];
						while ( m > 0 and fp < m_fringe_len and *pattern ==  db.fringe[lb*m_fringe_len+fp] ){
							++fp;
							++matched;
							++pattern;
							--m;
						}
						if( m > 0 and fp == m_fringe_len ){ // match until the end of the fringe and still a remaining pattern left
#ifdef OUTPUT_STATS
	if ( m_count_queries == m_X ){				
		cout << "(3a) m="<<m <<" more work to do"<< endl;
	}
#endif							
							size_type more_matched = 0;
#ifdef OUTPUT_STATS
							++m_count_gap_disk_access;
#endif							
							match_pattern(pattern, m, db.sa[lb] + matched, more_matched);
#ifdef OUTPUT_STATS
	if ( m_count_queries == m_X ){				
		cout << "more_matched=" << more_matched << endl;
				...
				if ( m > 0 ){
					++lb;
					while ( lb <= end and db.lcp[lb] > matched ){
						++lb;
					}
					if ( db.lcp[lb] < matched ){
						break;
					}	
				}
			}
...
#ifdef OUTPUT_STATS				
	if ( m_count_queries == m_X ){			
		cout<<"found pattern: matched = "<<matched<<" lb="<<lb<<" "<<" end="<<end<<endl; 				
	}
#endif				
				size_type rb = lb;
				while ( rb+1 <= end and matched <= db.lcp[rb+1] ){
					++rb;
				}
//cout<<"rb="<<rb<<endl;				
				return rb-lb+1;
		...
		size_type serialize(std::ostream &out)const{
			...
			written_bytes += util::write_member(m_fringe, out);
			written_bytes += util::write_member(m_fringe_len, out);
			return written_bytes;
		}	
		...
		void load(std::istream &in){
			...
			util::read_member(m_fringe, in);
			util::read_member(m_fringe_len, in);
			open_streams();
		}
		...
		void statistics()const{
			...
			while ( m_ext_idx.tellg() < end ){
				...
				fringe_in_megabyte += util::get_size_in_mega_bytes( db.fringe );
			...
			std::cout << "# fringe_len = " << m_fringe_len << std::endl;
			std::cout << "# fringe_type = " << m_fringe << std::endl;
			...
			std::cout << "# fringe_in_megabyte = " << fringe_in_megabyte << std::endl;
		}
		...
		void trie_nodes() const{
			...
			if ( !util::load_from_file(cst, tmp_fwd_cst_file_name.c_str()) ){
				...
				size_type dummy_cnt;
				get_block_info_and_mark_blocks(cst, dummy_bf, m_b, trie_nodes, dummy_cnt, dummy_cnt);	
