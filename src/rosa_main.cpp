...
void generate_index(tIndex &idx, const char *file_name, size_type b, 
			   fringe_type fringe = NO_FRINGE, size_type fringe_len=0, bool output_tikz=false, bool delete_tmp=false, 
			   const char *tmp_file_dir="./", const char *output_dir=".") {
	typedef bit_vector::size_type size_type;
	cerr << "call generate_index " << endl;

	string int_idx_file_name = tIndex::get_int_idx_filename(file_name,b,fringe_len, fringe, output_dir);
	string ext_idx_file_name = tIndex::get_ext_idx_filename(file_name,b,fringe_len, fringe, output_dir);
	...
	if ( !int_idx_stream or !ext_idx_stream )
	{
		...
		tIndex index(file_name, b, fringe, fringe_len, output_tikz, delete_tmp, tmp_file_dir, output_dir);
...
template<class tIndex>
void delete_index_files(const char *file_name, const char *output_dir, size_type b, fringe_type fringe = NO_FRINGE, size_type fringe_len=0) {
	string int_idx_file_name = tIndex::get_int_idx_filename(file_name,b,fringe_len, fringe, output_dir);
	string ext_idx_file_name = tIndex::get_ext_idx_filename(file_name,b,fringe_len, fringe, output_dir);
...
void benchmark(const char *file_name, size_type b, const char *pattern_file_name, const char *tmp_file_dir, const char *output_dir,
			   fringe_type fringe = NO_FRINGE, size_type fringe_len=0, bool repeated_in_memory_search=false,
	...
	{
		tIndex index;
		generate_index(index, file_name, b, fringe, fringe_len, false, false,
	...
	std::cout << "# fringe_len = " << index.fringe_len << std::endl;
	std::cout << "# fringe_type = " << index.fringe << std::endl;
...
void display_usage(char *command){
	...
	cout << " fringe_len       : the length of the fringe; default=0" << endl;
	cout << " fringe_type      : [right|no]; default=no" << endl;
...
int main(int argc, char *argv[]){
	...
	while ( 1 ){
		static struct option long_options[] = {
				...
				{"fringe_type", required_argument, 0, 'f'},
				{"fringe_len", required_argument, 0, 'l'},
	...
	if ( do_generate_index ){
		tIDX index;
		generate_index(index, input_file_name.c_str(), b, fringe, fringe_len, false, delete_tmp_file, tmp_file_dir.c_str(), output_dir.c_str() );
		return 0;
	}

	if ( output_tikz ){
		tIDX index;
		generate_index(index, input_file_name.c_str(), b, fringe, fringe_len, true, true, tmp_file_dir.c_str(), output_dir.c_str() );
		return 0;
	}

	if ( delete_index_file ){
		delete_index_files<tIDX>(input_file_name.c_str(), output_dir.c_str(), b, fringe, fringe_len);
		return 0;
	}

	if ( output_mem_info or output_statistics or output_Hk or output_trie_nodes ){
		tIDX index;
		string int_idx_file_name = tIDX::get_int_idx_filename(input_file_name.c_str(), b, fringe_len, fringe, output_dir.c_str());
		if ( util::load_from_file( index, int_idx_file_name.c_str() ) ){
			...
			std::cerr << "Have you specified threshold, fringe_type and fringe_len?" << endl;
			return 1;
		}
		return 0;
	}
	...
	if ( run_benchmark or run_benchmark_int or run_benchmark_ext  ){
		...
		benchmark<tIDX>( input_file_name.c_str(), b, pattern_file_name.c_str(), tmp_file_dir.c_str(),
			             output_dir.c_str(), fringe, fringe_len, repeated_in_memory_search,
